...
export default function UBLXmlTag(ublTagData) {
    let template = UBLtemplate;
    template = template.replace("SET_INVOICE_HASH", ublTagData.invoice_hash);
    template = template.replace("SET_SIGNED_PROPERTIES_HASH", ublTagData.signed_properties_hash);
    template = template.replace("SET_DIGITAL_SIGNATURE", ublTagData.digital_signature);
    template = template.replace("SET_CERTIFICATE", ublTagData.certificate_string);
    template = template.replace("SET_SIGNED_PROPERTIES_XML", ublTagData.signed_properties_xml);
    template = template.replace("SET_SigningTime", ublTagData.invoice_hash);
    console.log(template);
    return template.toString();
}
//# sourceMappingURL=UBLTag.js.map