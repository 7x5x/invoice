 ...
 */
export const generateQR = ({ invoice_xml, digital_signature, public_key, certificate_signature, }) => {
    // Hash
    ...
    const invoice_type = invoice_xml
        .get("Invoice/cbc:InvoiceTypeCode")?.[0]["@_name"].toString();
 ...
 * @returns String base64 encoded QR data.
 */
export const generatePhaseOneQR = ({ invoice_xml, }) => {
    // Extract required tags
    const seller_name = invoice_xml.get("Invoice/cac:AccountingSupplierParty/cac:Party/cac:PartyLegalEntity/cbc:RegistrationName")?.[0];
    const VAT_number = invoice_xml
        .get("Invoice/cac:AccountingSupplierParty/cac:Party/cac:PartyTaxScheme/cbc:CompanyID")?.[0]
...
const TLV = (tags) => {
    ...
    tags.forEach((tag, i) => {
        ...
        const current_tlv_value = Buffer.from([
            i + 1,
